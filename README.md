...
- `GET /health`: service health check with RDKit, Meeko, and molscrub availability
...
`status` is `"ok"` when all critical dependencies are available and `"degraded"` when one or more are missing.
